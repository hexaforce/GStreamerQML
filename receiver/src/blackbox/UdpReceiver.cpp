...
CommonTelemetrySocket::CommonTelemetrySocket(const std::string &ip, unsigned short port) : udp_ip(ip), udp_port(port) {
  // Initialize socket, bind, etc.
  setupUdpSocket(); // setupUdpSocket 関数を呼び出す
}

CommonTelemetrySocket::~CommonTelemetrySocket() { close(udp_socket); }
