...
CommonTelemetrySocket::CommonTelemetrySocket(QObject *parent) : QObject(parent) { udpSocket = new QUdpSocket(this); }
...
void CommonTelemetrySocket::messageSend(const QString &message, const QString &host, int port) { udpSocket->writeDatagram(message.toUtf8(), QHostAddress(host), port); }
